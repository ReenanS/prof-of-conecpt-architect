...
> **Revisão 20:** nova solução de curtíssimo prazo recomendada: status de sincronização no S3 (bucket aprovado pela governança), endpoint do HUB Pessoas, verificação no HUB Reserva, callback como CDC de aplicação e garantia de ordem por pessoa (6.14 a 6.18). A 6.9 passa a ser rede de proteção obrigatória. Plano (6.7), resumo, matriz, diagrama 15.3, fluxos (16 e 17.1), idempotência, observabilidade, LGPD, roadmap e riscos atualizados.
...
- (a) **não criam banco**, por restrição da empresa **[FATO]**. A única persistência nova é um **bucket S3 sem dados cadastrais, aprovado pela governança** **[FATO]**. Aproveitam leituras que já existem;
...
| **Curtíssimo prazo recomendado** (6.15 a 6.18) | O HUB Pessoas grava o **status da sincronização no S3** (`PENDENTE` → `CONCLUIDA`/`FALHOU`), atualizado pelo consumidor e pelo callback da Altera (6.14). O **HUB Reserva consulta o status** (endpoint 6.16) e só chama a Contratação quando está `SINCRONIZADO`. A **ordem por pessoa** é garantida por fila FIFO, timeout SQL e conferência (6.18). O HUB publica antes do 200 (6.10). | AWS (Core Pessoas, Core Reserva) + PaaS (Altera) + MFEs; **bucket S3 aprovado** | Médio |
| Curtíssimo prazo — **rede de proteção obrigatória** (6.9) | A **Contratação compara** o **cadastro único** com o **Newcon** antes da Confirma. Se divergirem, devolve 409 e o HUB Reserva faz backoff. Cobre escrita dupla, desordem e perda de status. | PaaS (Contratação) | Baixo |
...
**A resposta à pergunta central (seção 14.1):** Pessoas publica o **fato** `AtualizacaoCadastralConcluida`. A Jornada **deriva** o estado `PRONTA_PARA_CONTRATACAO`. Nem o HTTP 200 nem um evento `PessoaProntaParaContratacao` emitido por Pessoas devem autorizar a contratação. **No hotfix**, esse fato é materializado no **status `CONCLUIDA` no S3** (6.15), consultado pelo HUB Reserva, com a **igualdade cadastro único × Newcon** (6.9) como confirmação final imediatamente antes da Confirma.
...
| **h) Comparação cadastro único × Newcon na Contratação (6.9)** | A Contratação compara as duas leituras que já faz antes da Confirma. | **Sim, sem banco**, de forma determinística. | **Rede de proteção obrigatória** da solução recomendada. |
| **i) Status de sincronização no S3 + verificação no HUB Reserva (6.15 a 6.18)** | Status consultável, alimentado pelo consumidor e pelo callback; o HUB Reserva só chama a Contratação quando está sincronizado. | **Sim**, com a 6.9 como rede de proteção. | **Recomendada** (bucket aprovado pela governança). |

### 6.2 Solução com tabela de status + pré-condição no HUB Reserva (**descartada pela restrição de banco**; mantida como referência)

> Esta solução era a recomendada até a revisão 6. Com DynamoDB, foi descartada pela restrição de não criar banco **[FATO]**. **A mesma lógica foi retomada com um objeto no S3** (bucket aprovado pela governança), nas seções **6.15 a 6.18**, que são a recomendação atual, com a 6.9 como rede de proteção. O conteúdo abaixo fica como referência de regras (transições, pré-condição, filas).
...
> **Nota (revisão 10):** esta tabela considera a solução com tabela de status (6.2). Para a solução recomendada (6.15 a 6.18, com a 6.9 como rede de proteção), o plano por time está na **seção 6.7**.
...
### 6.7 Plano de ação do hotfix por time (atualizado para a solução 6.15 a 6.18, com a 6.9 e a 6.10)

> As versões anteriores deste plano, baseadas na 6.2 e na 6.9 isoladas, foram substituídas.

**Time Core de Pessoas**
1. Bucket S3 do status: privado, SSE-KMS, ciclo de vida e CloudTrail de dados (6.15).
2. HUB Pessoas: gravar no cadastro único, gravar `PENDENTE` no S3, **publicar na fila** e só então responder 200; em falha, 5xx (6.10 e 6.15).
3. Consumidor do cadastro:
   - enviar `X-Id-Solicitacao`, `X-Versao-Cadastro` e `X-Correlation-Id` à Altera;
   - aplicar a classificação de retornos (6.4);
   - atualizar o status com escrita condicional e a conferência da 6.18.
4. Endpoints de sincronização e de reparo (6.16).
5. Fila do cadastro **FIFO por pessoa** (6.18) e DLQ com a Lambda que marca `FALHOU` técnico.
6. Lambdas do callback (API Gateway → validação → SQS → atualização do status) e notificação de evento do S3 (6.14).

**Time Core de Reserva**
1. **Primeiro:** visibility timeout da fila de reserva em 60–180 s e cliente HTTP em ~20 s.
2. Verificação no consumidor (6.17), atrás de feature flag, começando em **modo observação**.
3. Tratamento de `AGUARDANDO`, `ATRASADO`, `FALHOU`, 409, 504 e expiração.
4. `maxReceiveCount`, retenção e DLQ compatíveis com a janela X.

**Time da Contratação**
1. Comparação cadastro único × Newcon antes da Confirma (6.9), com especificação de comparação e modo observação.
2. Devolver `409 CADASTRO_NAO_CONVERGIU` quando o bloqueio estiver ligado.
3. Orçamento interno ≤ 10–12 s, incluindo a leitura do cadastro único (6.5).

**Time do PaaS (Altera Cliente)**
1. Receber e devolver os headers; enviar o callback após o commit (6.4 e 6.14).
2. Timeout de comando SQL menor que o Kong (6.18).

**Seu time (MFEs)**
1. MFE Cadastro: tratar o 5xx do HUB como erro recuperável, com reenvio sem redigitar. Precisa estar em produção **antes ou junto** do item 2 do Core de Pessoas.
2. MFE Reserva: polling na rota externa do endpoint (6.16), com backoff e limite de duração; telas de "aguardando", "avisaremos você", `FALHOU` e expirada.
3. `correlationId` da jornada.

**Negócio**
- Janela X, SLO de sincronização e comunicação ao cliente durante a espera.

**Ordem sugerida:**
1. Timeouts do Core de Reserva (item 1). Imediato e independente.
2. Bucket e gravação de status (Core de Pessoas 1 a 3), junto com o item 1 do seu time. Nesse ponto, o status é gravado mas ninguém o lê.
3. 6.9 em modo observação na Contratação, e verificação do HUB Reserva em modo observação.
4. Medir a concordância: o endpoint dizer `SINCRONIZADO` deve coincidir com a 6.9 dizer "igual".
5. Ligar o bloqueio da 6.9 e depois a verificação do HUB Reserva.
6. Em paralelo: FIFO, callback, reparo e polling do MFE.

**Rollback:** desligar as flags. O fluxo volta ao atual.
...
**Recomendação (atualizada na revisão 9; ver também a revisão 20):** com a confirmação de que a Contratação já consulta o cadastro único, a **comparação direta da seção 6.9** substituiu o token. Desde a revisão 20, a 6.9 é a **rede de proteção** da solução recomendada (6.15 a 6.18). O token fica como **plano B**, se a comparação entre os dois modelos se mostrar inviável. O texto abaixo continua válido para esse caso:
...
### 6.9 Verificação de convergência na Contratação — **rede de proteção obrigatória** (premissas validadas)

> **Nota (revisão 20):** esta seção foi a solução recomendada até a revisão 19. Com a aprovação do bucket S3, a recomendação passou a ser a **6.15 a 6.18**, e a 6.9 continua **obrigatória** como rede de proteção.
...
**Onde está a memória na 6.9.** Ela funciona sem banco novo porque usa **memórias que já existem**:
...
| **d) Objeto de status no S3** | O consumidor e o callback atualizam `atualizacao-cadastral/{idPessoa}.json` | **Aprovada pela governança** **[FATO]**. É a solução recomendada (6.15). |

> **Atualização (revisão 20):** com o bucket S3 aprovado, o callback passa a alimentar um **status consultável** (6.14 e 6.15), que se torna a trava do HUB Reserva. O parágrafo abaixo descreve a recomendação anterior.

**Recomendação anterior:** manter a **6.9 como hotfix**. Implementar o callback como **complemento de curto prazo**, para o aviso de rejeição (400) e as métricas de sincronização.
...
### 6.14 Callback da Altera como "CDC de aplicação" (primeiro momento)

**Proposta.** Até o CDC do Newcon existir, o callback da Altera Cliente (6.4) funciona como fonte de eventos de mudança. É um "CDC de aplicação": o evento é emitido pelo código logo após o commit, o que é válido porque o 200 da Altera só volta depois do commit **[FATO]**.

| Característica | CDC real (log do SQL Server) | Callback da Altera |
|---|---|---|
| Origem do sinal | Commit no banco | Código da Altera, após o commit |
| Garantia de emissão | Toda mudança confirmada | Pode se perder (pod reinicia após o commit, API Gateway fora) |
| Cobertura | Qualquer escrita no Newcon | Só as escritas feitas pela Altera Cliente |
| Ordem | Pela posição no log | Não garantida |
| Depende do fornecedor | Sim | **Não** |

**Ajustes para ele se comportar como CDC:**
1. **Versão no evento.** O consumidor do cadastro envia à Altera, em header, o `idPessoa`, o `idSolicitacao` e a `dataUltimaAlteracao` do cadastro único, e a Altera devolve os três no callback.
2. **Dois emissores, um só resultado.** O consumidor do cadastro (ao receber o 200) e a Altera (pelo callback) atualizam o **mesmo status** (6.15). As regras de escrita condicional tornam a segunda atualização inofensiva.
3. **Distribuição.** Caminho: API Gateway → Lambda de validação → SQS → Lambda que atualiza o status no S3. As **notificações de evento do S3** (S3 → EventBridge) publicam a mudança de status para os consumidores de aviso: notificação ao cliente e métricas.
4. **Contrato estável, fonte trocável.** O evento publicado (ex.: `AtualizacaoCadastralAplicadaNoNewcon`) já nasce no formato que a ponte do CDC publicará no médio prazo. Quando o CDC chegar, muda só quem emite.

**O callback sozinho não é a trava da contratação** (6.13): um evento não é consultável depois. Ele passa a alimentar o **status no S3 (6.15)**, e é esse status, consultável, que serve de trava.

### 6.15 Status de sincronização no S3 — **solução de curtíssimo prazo recomendada**

**Viabilidade confirmada:** a governança **aprovou** um bucket S3 sem dados cadastrais, com o `idPessoa` e apagamento automático **[FATO]**. É a estrutura da tabela de status (6.2), com o S3 no lugar do DynamoDB.

**Um objeto por pessoa:** `atualizacao-cadastral/{idPessoa}.json`, sobrescrito a cada nova solicitação.

```json
{
  "idPessoa": "123456",
  "idSolicitacao": "01J9Z6K8H4X3...",
  "status": "PENDENTE",
  "versaoCadastroUnico": "2026-09-21T14:03:12Z",
  "solicitadaEm": "2026-09-21T14:03:12.481Z",
  "atualizadaEm": "2026-09-21T14:03:12.481Z",
  "motivoFalha": null,
  "reparoSolicitadoEm": null,
  "fonte": "hub-pessoas",
  "correlationId": "jornada-7f3c..."
}
```

- **`idSolicitacao`** é um ULID gerado pelo HUB a cada gravação, e é ele que identifica a versão. Isso é necessário porque a `dataUltimaAlteracao` do cadastro único tem **precisão de segundos** **[FATO]**: duas gravações no mesmo segundo teriam a mesma data.
- **`versaoCadastroUnico`** é informativa: serve para métrica, para o reparo e para a conferência da 6.18.
- **Não há campo de TTL:** o apagamento vem da regra de ciclo de vida do bucket (ex.: 30 dias, sempre acima da janela X).

**Quem escreve o quê:**

| Momento | Quem | Escrita |
|---|---|---|
| Antes do 200 | HUB Pessoas | `PENDENTE` com um novo `idSolicitacao` |
| Altera responde 200 | Consumidor do cadastro | `CONCLUIDA`, **só se** o `idSolicitacao` do objeto for o dele (e passar na conferência da 6.18) |
| Altera responde 400 | Consumidor do cadastro | `FALHOU` com o código de negócio, na mesma condição |
| 5xx ou 504 | Ninguém | Continua `PENDENTE`; a fila faz o retry |
| Callback após o commit | Lambda do callback | Mesma regra do consumidor; se já estiver `CONCLUIDA`, não faz nada |
| Mensagem vai para a DLQ | Lambda da DLQ | `FALHOU` com motivo técnico |
| Redrive com sucesso | Consumidor | Pode voltar de `FALHOU` (técnico) para `CONCLUIDA` |
| Reparo (6.10 e 6.18) | HUB Pessoas | Novo `PENDENTE`, com novo `idSolicitacao` |

**Ordem de operações no HUB Pessoas:**
1. Grava no cadastro único.
2. Grava o `PENDENTE` no S3.
3. Publica na fila (6.10).
4. Responde 200.

Se o passo 2 ou o 3 falhar, o HUB responde **5xx** e o cliente reenvia. O reenvio gera um novo `idSolicitacao`.

**Escrita condicional.** Todas as transições, exceto o `PENDENTE` do HUB, seguem o mesmo padrão:
1. Ler o objeto e guardar a ETag.
2. Conferir o `idSolicitacao` e o status.
3. Gravar com `If-Match: <ETag>`. Se voltar 412, ler de novo e reavaliar.

**[HIPÓTESE]** O suporte do S3 à escrita condicional por ETag deve ser confirmado no SDK e na região. Se não estiver disponível, sobra uma janela de milissegundos de corrida, coberta pela 6.9.

**A 6.9 continua obrigatória como rede de proteção.** O status pode divergir do dado em três casos: escrita dupla (o Newcon grava e o status não), desordem (6.18) e envios simultâneos da mesma pessoa. Só a comparação cadastro único × Newcon, feita imediatamente antes da Confirma, percebe esses casos.

### 6.16 Endpoint de sincronização (HUB Pessoas)

**Rota:** `GET /v1/pessoas/{idPessoa}/sincronizacao-newcon`

| Exposição | Quem chama | Autenticação | Resposta |
|---|---|---|---|
| **Interna** (serviço a serviço, na AWS) | Consumidor do HUB Reserva | IAM ou credencial de serviço | Completa |
| **Externa** (via borda/BFF) | MFE Reserva (polling) | Token do cliente; o `idPessoa` é derivado ou validado pela sessão | Reduzida |

**Parâmetros:**

| Parâmetro | Onde | Obrigatório | Uso |
|---|---|---|---|
| `idPessoa` | caminho | Sim | Chave do objeto no S3 |
| `idSolicitacao` | query | Não | O MFE informa o ID recebido no 200. Se o objeto já tiver um ID mais novo, a resposta indica `substituida: true` e passa a refletir a solicitação mais recente |
| `X-Correlation-Id` | header | Sim (interna) | Rastreio ponta a ponta |

**Lógica:**
1. Um GET no objeto do S3, cuja leitura é fortemente consistente, **sem cache**.
2. **Sem efeitos colaterais**: o reparo tem endpoint próprio.
3. Tradução do objeto numa `situacao`:

| Objeto no S3 | `situacao` |
|---|---|
| Não existe | `SEM_PENDENCIA` |
| `CONCLUIDA` | `SINCRONIZADO` |
| `PENDENTE`, com `solicitadaEm` dentro do SLO | `AGUARDANDO` |
| `PENDENTE`, acima do SLO | `ATRASADO` |
| `FALHOU` por negócio (400) | `FALHOU` |
| `FALHOU` técnico (DLQ) | `ATRASADO` |

**Resposta da rota interna** (HTTP 200 para qualquer situação conhecida, inclusive `SEM_PENDENCIA`):
...
- **`tentarNovamenteEmSegundos`** cresce com a idade da solicitação (ex.: 10 s no primeiro minuto, 30 s até 10 min, 60 s depois).
- **Rota externa:** devolve só `situacao`, `tentarNovamenteEmSegundos` e, em `FALHOU`, um código traduzido para a tela.
- **Erros:** 400 (`idPessoa` inválido), 401/403, 429 (limite de requisições), 5xx ou timeout (o chamador trata como "indisponível").
- **Metas:** p99 de dezenas de milissegundos; timeout de 2 s no cliente, com no máximo 1 retentativa; IAM só de leitura no bucket.

**Endpoint de reparo:** `POST /v1/pessoas/{idPessoa}/ressincronizacao`.
- Lê o cadastro único, grava um novo `PENDENTE` e republica.
- É idempotente: grava `reparoSolicitadoEm` por escrita condicional e **ignora novos pedidos na mesma janela** (ex.: 30 min).

### 6.17 HUB Reserva: verificação antes da Contratação

**Como ele aguarda.** A própria **mensagem da reserva na fila espera**, sem thread bloqueada e sem estado novo. Quando a resposta é `AGUARDANDO`, o consumidor não apaga a mensagem e chama `ChangeMessageVisibility` com o atraso sugerido. Como a consulta vai ao S3, e não ao SQL Server, o intervalo pode ser curto (ex.: teto de 60 s nos primeiros 30 min, 5 min depois).

```text
receber mensagem da reserva
idade = agora - SentTimestamp da mensagem
se idade > janela X: reserva EXPIRADA, avisar cliente, apagar mensagem, alertar; fim
se gate desligado: chamar Contratação como hoje; fim
se modo observação: consultar endpoint, registrar; chamar Contratação como hoje; fim

consultar GET sincronizacao-newcon (timeout 2 s)
  SINCRONIZADO | SEM_PENDENCIA -> chamar Contratação
  AGUARDANDO                   -> adiar mensagem (tentarNovamenteEmSegundos + jitter)
                                  reserva visível ao MFE como AGUARDANDO_CADASTRO
  ATRASADO                     -> POST ressincronizacao (idempotente)
                                  chamar Contratação (a 6.9 decide)
  FALHOU                       -> reserva FALHOU_CADASTRO, avisar cliente, apagar mensagem
  erro ou timeout do endpoint  -> chamar Contratação (a 6.9 decide), alertar

resultado da Contratação
  200                          -> sucesso, apagar mensagem
  409 CADASTRO_NAO_CONVERGIU   -> adiar mensagem
                                  se o endpoint tinha dito SINCRONIZADO: POST ressincronizacao + alerta
  504 do Kong                  -> resultado incerto: registrar, alertar, deixar reprocessar
  outros 5xx                   -> reprocessamento normal
```

**O que muda no HUB Reserva:**
1. **Mensagem da reserva:** passa a carregar o `idPessoa` corporativo, validado contra a sessão **[HIPÓTESE: confirmar se já carrega]**.
2. **Consumidor:** consulta ao endpoint antes da Contratação, backoff e tratamento dos resultados 409, 504, `FALHOU` e expiração.
3. **Situações novas da reserva, visíveis ao polling do MFE:** `AGUARDANDO_CADASTRO`, `FALHOU_CADASTRO` e `EXPIRADA` **[HIPÓTESE: depende de como o status da reserva é exposto hoje]**.
4. **Configuração da fila:**
   - visibility timeout base de 60–180 s;
   - `maxReceiveCount` compatível com a janela X, porque cada espera conta como recebimento. A expiração por idade deve vencer antes da DLQ;
   - retenção maior que a janela X.
5. **Configurações novas:** SLO, janela X, teto do backoff, URL e timeout do endpoint, flags de gate e de observação.
6. **Cliente HTTP da Contratação:** ~20 s (6.5).

**Linha do tempo típica (sincronização em 10 min):** a reserva é consultada a cada 30–60 s, só com leituras no S3, e segue no máximo cerca de 1 minuto depois de o status virar `CONCLUIDA`.

### 6.18 Atualizações seguidas da mesma pessoa (ex.: 30 s de diferença)

**O status no S3 se protege sozinho.** A conclusão de uma solicitação antiga não altera um objeto que já está com o `idSolicitacao` mais novo.

**O risco real é a ordem no Newcon:** S1 recebe 504 do Kong com o SQL Server lento, S2 é aplicada e marcada `CONCLUIDA`, e depois S1 é reaplicada. O Newcon regride para S1 enquanto o status diz `CONCLUIDA (S2)`.

**Quatro camadas de garantia:**
1. **Fila do cadastro FIFO por pessoa** (Core de Pessoas). `MessageGroupId = idPessoa` e `MessageDeduplicationId = idSolicitacao`: a mensagem de S2 só é entregue depois que a de S1 for apagada. Exige criar uma fila nova e migrar, e confirmar que o throughput atende o volume de pico **[pergunta aberta]**.
2. **Nenhuma gravação "fantasma"** (time do PaaS). Timeout de comando SQL na Altera **menor que o Kong** (ex.: 10–12 s), e visibility timeout da fila do cadastro bem acima disso (ex.: 60–120 s).
3. **Conferência antes de marcar `CONCLUIDA`** (Core de Pessoas). Depois do 200, o consumidor relê a `dataUltimaAlteracao` do cadastro único. Se ela for mais nova que a versão aplicada, ele não marca e deixa a solicitação mais nova seguir.
4. **Rede de proteção** (Contratação e Core de Reserva). A 6.9 devolve 409. Se o endpoint tinha dito `SINCRONIZADO`, o HUB Reserva chama o reparo, que grava um novo `PENDENTE` e republica. A reserva volta a esperar e se corrige sozinha.

**Depois:** a mensagem só com o identificador (6.11) e a validação de versão no Newcon (fornecedor).

**Caso de borda:** uma alteração feita **depois** de a reserva já ter sido processada é uma **[DECISÃO DE NEGÓCIO]**: aceitar, ou bloquear a edição do cadastro durante a contratação.
...
3. **Callback da Altera Cliente** (6.4, 6.13 e 6.14). Com o bucket S3 aprovado, ele **também atualiza o status** (6.15). Como aviso (time do PaaS e Core de Pessoas). O callback publica `AtualizacaoCadastralConcluida` / `AtualizacaoCadastralFalhou` num SNS ou EventBridge, sem guardar nada. Serve para:
...
| **Status no S3 + verificação no HUB Reserva (6.15 a 6.18)** | M | B–M | 0 síncronas novas (callback assíncrono pelo canal oficial) | **Alta** (esperas fora do SQL Server; `ATRASADO` leva à 6.9) | **Alta** (com a 6.9) | **Alta** (situação consultável por pessoa) | M |
...
**Leitura:** isto não é um ranking. A **comparação da 6.9** é a que mais melhora a consistência por unidade de esforço, mas deixa as esperas na Contratação e no SQL Server. O **status no S3 (6.15 a 6.18)**, viável depois da aprovação do bucket, move as esperas para a AWS, permite polling barato no MFE e detecta a rejeição por regra de negócio na hora. Por isso, a recomendação é **as duas juntas**: o S3 como trava principal e a 6.9 como rede de proteção. As alternativas com DynamoDB (c1, c2) continuam descartadas.
...
| **Status em armazenamento de objeto (S3)** | Estado consultável por chave, sem banco | **Alta** (6.15): um objeto por pessoa, com escrita condicional por ETag | Lógica de concorrência na aplicação; sem consulta por atributo | Quando é preciso consultar por atributo ou fazer transações entre vários itens |
...
### 15.3 Solução de curtíssimo prazo recomendada (6.15 a 6.18, com a 6.9 e a 6.10)

```mermaid
sequenceDiagram
  autonumber
  box AWS
  participant MC as MFE Cadastro/Reserva
  participant HP as HUB Pessoas (+ endpoint)
  participant CU as Cadastro único
  participant S3 as S3 status
  participant QC as SQS Cadastro FIFO + consumidor
  participant HR as HUB Reserva (SQS + consumidor)
  end
  participant K as Kong
  box PaaS
  ...
  MC->>HP: dados cadastrais
  HP->>CU: grava (sync)
  CU-->>HP: idPessoa + dados + dataUltimaAlteracao
  HP->>S3: PENDENTE (idSolicitacao S1)
  HP->>QC: publica S1
  HP-->>MC: 200 + idPessoa + idSolicitacao
  MC->>HR: reserva
  HR->>HP: GET sincronizacao-newcon
  HP->>S3: lê status
  HP-->>HR: AGUARDANDO (tentar em 30 s)
  Note over HR: adia a mensagem, sem tocar no SQL Server
  QC->>K: Altera (headers S1, versão)
  K->>AC: encaminha
  AC->>DB: SP upsert (commit)
  AC-->>QC: 200
  QC->>CU: confere dataUltimaAlteracao
  QC->>S3: CONCLUIDA se idSolicitacao == S1 (If-Match)
  AC->>S3: callback via API Gateway (mesma regra, sem efeito se já concluído)
  HR->>HP: GET sincronizacao-newcon
  HP-->>HR: SINCRONIZADO
  HR->>K: Contratação
  K->>CT: encaminha
  CT->>CU: lê (via API Gateway)
  CT->>DB: Consulta e compara (6.9) - IGUAL
...
## 16. Fluxos de sucesso (com a solução recomendada: 6.15 a 6.18, com a 6.9 e a 6.10)

1. **Cliente existente, sem alteração de dados:** não há objeto no S3, ou ele já está `CONCLUIDA`. O endpoint responde `SEM_PENDENCIA` ou `SINCRONIZADO`, a comparação da 6.9 dá "igual" e o fluxo fica idêntico ao atual.
2. **Cliente existente, sincronização em ~10 minutos:** o endpoint responde `AGUARDANDO` e o HUB Reserva adia a mensagem, só com leituras no S3. Quando o consumidor marca `CONCLUIDA`, a próxima consulta responde `SINCRONIZADO`, a 6.9 confirma "igual" e a proposta é criada. O MFE mostra "aguardando confirmação" pelo polling e, passado o tempo de tela, "avisaremos você".
3. **Cliente novo:** mesmo fluxo do item 2. Enquanto o status for `PENDENTE`, a Contratação não é chamada.

---

## 17. Fluxos de falha (6.9 + 6.10 atuando sozinhas; o que o curto prazo, seção 7.1, acrescenta; o efeito do status no S3 está na 17.1)
...
### 17.1 O que muda nos cenários com o status no S3 (6.15 a 6.18)

> Os cenários acima descrevem a 6.9 atuando sozinha. Com o status no S3, a 6.9 continua valendo como rede de proteção. A tabela resume o que muda em cada cenário.

| Cenário | Com o status no S3 |
|---|---|
| 1. Gravação demora 10 min | A reserva espera no HUB Reserva (`AGUARDANDO`), só com leituras no S3; a Contratação e o SQL Server não são acionados durante a espera |
| 2. Indisponibilidade de 2 h | Igual ao 1. Após o SLO, `ATRASADO` leva à 6.9 e ao reparo |
| 3. Indisponibilidade de 2 dias | A DLQ marca `FALHOU` técnico, que o endpoint trata como `ATRASADO` (6.9 + reparo). O redrive pode levar a `CONCLUIDA`. A reserva expira na janela X |
| 4. Cadastro processado duas vezes | A escrita condicional torna a segunda marcação inofensiva |
| 5. Mensagem da reserva duplicada | Sem mudança: a proteção é a da 6.5 e da 7.1 (item 4) |
| 6. Fora de ordem | Coberto pelas quatro camadas da 6.18 |
| 7. Resposta ou callback perdidos | Dois emissores do status. Se os dois falharem, fica `PENDENTE` → `ATRASADO` → 6.9 |
| 8. Contratação antes da atualização | Barrada já no HUB Reserva (`AGUARDANDO`), antes da 6.9 |
| 9. 504 na chamada à Contratação | Sem mudança |
| 10. Abandono | O status conclui normalmente; o objeto é apagado pelo ciclo de vida |
| 11. Escrita dupla (publicação falha) | Uma falha ao gravar o S3 ou ao publicar leva a 5xx, e o cliente reenvia. Se o HUB cair entre os passos, o `PENDENTE` sem mensagem vira `ATRASADO` e aciona o reparo |
| 12. Rejeição por regra de negócio (400) | O status vira `FALHOU` na hora. O HUB Reserva encerra a reserva e o cliente é avisado (notificação do S3 → EventBridge) |
...
## 19. Idempotência e consistência (solução recomendada: sem banco, com o status no S3)
...
| **Status no S3** | Escrita condicional (`If-Match` por ETag) e transições só para o `idSolicitacao` mais recente | AWS | **[HIPÓTESE]** suporte a `If-Match` no SDK e na região |
| **Ordem por pessoa** | Fila do cadastro **FIFO** (`MessageGroupId = idPessoa`), timeout SQL menor que o Kong e conferência antes de `CONCLUIDA` (6.18) | AWS + PaaS | Volume de pico (pergunta aberta) |
...
| Consultas ao endpoint por `situacao` | Tamanho da espera e da fila de pendentes | `ATRASADO` acima do normal |
| **409 depois de `SINCRONIZADO`** | Status e dado divergiram (regressão, escrita dupla ou mapeamento) | Qualquer valor acima de 0 |
| Idade dos objetos `PENDENTE` (via S3 Inventory) | Pendências esquecidas | Acima do SLO |
...
- **Bucket de status (6.15):** privado, bloqueio de acesso público, SSE-KMS, ciclo de vida (ex.: 30 dias), CloudTrail de dados; escrita só pelas funções do Core de Pessoas e leitura só pelo endpoint. Guarda o `idPessoa` (pseudonimizado) e **nenhum dado cadastral**. Aprovado pela governança **[FATO]**.
...
|---|---|---|---|---|---|---|---|
| **Agora (hotfix)** | Parar a contratação com dado antigo e tirar a espera do SQL Server | **Status no S3 + endpoint + verificação no HUB Reserva** (6.15 a 6.17); ordem por pessoa (6.18); **6.9 como rede de proteção**; publicar antes do 200 (6.10); callback como CDC de aplicação (6.14); hierarquia de timeouts | AWS (Core Pessoas, Core Reserva) + PaaS (Contratação, Altera) + MFEs | Médio | Baixo a médio (flags e modo observação) | Janela X; SLO; volume de pico para a FIFO; suporte a `If-Match` | Desligar as flags da verificação e do bloqueio |
...
| Status no S3 divergir do dado no Newcon | A 6.9 como rede de proteção; reparo automático após 409 com `SINCRONIZADO` (6.17) |
| Três escritas em sequência no HUB antes do 200 (cadastro único, S3, fila) | 5xx em qualquer falha, reenvio sem redigitar no MFE, `PENDENTE` órfão vira `ATRASADO` e aciona o reparo |
| Troca da fila do cadastro para FIFO | Validar o throughput contra o pico; migração com as duas filas em paralelo |
| Endpoint externo expondo status de outra pessoa | `idPessoa` derivado ou validado pela sessão; limite de requisições |
...
24. ~~A governança aceita o bucket S3 de status?~~ **Respondida: sim.**
25. ~~Qual é a precisão da `dataUltimaAlteracao`?~~ **Respondida: segundos.** Por isso, o `idSolicitacao` identifica a versão.
26. Qual é o volume de pico de atualizações cadastrais por segundo? (Valida a fila FIFO da 6.18.)
27. A mensagem da reserva já carrega o `idPessoa` corporativo, ou só o CPF/CNPJ?
28. Como o MFE acompanha hoje o resultado da reserva: o HUB Reserva guarda esse status em algum lugar? (Define onde entram `AGUARDANDO_CADASTRO`, `FALHOU_CADASTRO` e `EXPIRADA`.)
29. O consumidor do HUB Reserva roda em Lambda ou em ECS?
30. Existe um serviço corporativo de notificação (e-mail/push) para os avisos da 6.14?
31. O SDK e a região suportam escrita condicional por ETag no S3?
32. Qual é o timeout de comando SQL configurado hoje na Altera Cliente?
