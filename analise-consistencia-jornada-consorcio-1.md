...
| **Em paralelo, já** | Pedir ao fornecedor: CDC na tabela de pessoa, detecção de proposta duplicada e view "existe proposta para esta reserva?" (a coluna de última alteração passou a ser opcional). Revisar a hierarquia de timeouts. | Fornecedor + AWS + PaaS | Depende do fornecedor |
...
4. **Idempotência da contratação usando o Newcon como estado** (time da Contratação). Antes da Confirma, a Contratação verifica **se já existe proposta para aquela reserva** no Newcon. Se existir, devolve o resultado sem confirmar de novo.
   ...
   - Depende de uma view ou procedure que responda "existe proposta para esta reserva?" (pergunta 5 da seção 23 e pedido ao fornecedor na 6.3).
